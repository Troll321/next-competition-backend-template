"use client";
import { getLoginURL_SC, getLogoutURL_SC } from "@/api/authentication/both";
import { useUser_C } from "@/api/authentication/client";
import VerifyEmailButton from "@/components/functional/VerifyEmailButton";

function LoginButton() {
    return (
        <a href={getLoginURL_SC("/playground/integrated")} className="button login">
            Log In
        </a>
    );
}
...
function LogoutButton() {
    return (
        <a
            href={getLogoutURL_SC("/app_api/playground/authenticationAPI")}
...
function Profile() {
    ...
    return (
        <div className="profile-card action-card">
            ...
            <h1>Is Email verified? {user.email_verified ? "YES" : "no"}</h1>
            {user.email_verified ? <></> : <VerifyEmailButton />}
        </div>
    );
}
